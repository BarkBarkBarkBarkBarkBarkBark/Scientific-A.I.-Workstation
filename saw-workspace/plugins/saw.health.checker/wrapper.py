...
Reads saw-workspace/machine-context/api_endpoints.json and probes each listed endpoint.
By default it avoids potentially expensive/side-effectful checks (AI calls, writes, plugin execution)
unless enabled via params.
...
def _sample_body(body_spec: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    # Add a nonce when possible to make debugging easier (and to avoid caching).
    if "nonce" in out and isinstance(out.get("nonce"), str):
        out["nonce"] = str(uuid.uuid4())
    return out


def _http_request(method: str, url: str, body: dict[str, Any] | None, timeout_sec: float) -> tuple[int | None, str, int, str | None]:
...
def _is_plugin_exec_endpoint(path: str) -> bool:
    return path.endswith("/plugins/execute") or (path.startswith("/api/plugins/") and path.endswith("/run"))


def main(inputs: dict, params: dict, context) -> dict:
    endpoints_rel = str(((inputs or {}).get("endpoints_json") or {}).get("data") or "machine-context/api_endpoints.json").strip()
    ...
    for svc in services:
        ...
        # Allow env override (useful when ports differ)
        if base_env and os.environ.get(base_env):
            base_url = str(os.environ.get(base_env) or "").strip()

        # Vite proxy service is "same-origin" in the json; here we need an absolute URL.
        if service_id == "vite_openai_proxy":
        ...
        for ep in endpoints:
            ...
            # Skip endpoints that are likely to be side-effectful unless enabled.
            ...
            # Multipart upload endpoint isn't meaningfully probeable without a file.
            ...
            # Interpret results: 2xx/3xx pass; 4xx warn (endpoint exists but request may be invalid); 5xx fail.
            if err is not None:
                item["level"] = "fail"
                totals["fail"] += 1
            elif status is None:
    ...
    return {"report": {"data": report, "metadata": {"plugin": "saw.health.checker"}}}
